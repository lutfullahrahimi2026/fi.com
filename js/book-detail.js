(function () {
  ...
  if (!detailEl || typeof BOOKS_DATA === "undefined") return;

  const params = new URLSearchParams(window.location.search);
  const slug = params.get("slug") || BOOKS_DATA[0].slug; // default to first book for bare /book.html links
  ...
  const downloadHTML = book.pdfUrl
    ? `<a href="${book.pdfUrl}" download class="btn btn-secondary">Download</a>`
    : `<span class="btn btn-secondary" aria-disabled="true" title="Coming soon" style="opacity: 0.5; cursor: not-allowed;">Download</span>`;
  ...
  const related = BOOKS_DATA.filter((b) => b.series === book.series && b.slug !== book.slug).slice(0, 4);
